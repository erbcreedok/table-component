...
export const FilterOptionMenu = <TData extends Record<string, any> = {}>({
  ...
  const internalFilterOptions = useMemo(
    () =>
      filterOptions(localization).filter((filterOption) =>
        columnDef
  ...
  const handleSelectFilterMode = (option: Table_FilterOption) => {
    if (header && column) {
        ...
        internalFilterOptions.map(
          ({ option, label, divider, symbol }, index) => (
            <MenuItem
              divider={divider}
              key={index}
              onClick={() => handleSelectFilterMode(option as Table_FilterOption)}
